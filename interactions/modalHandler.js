...
module.exports = async (interaction, client) => {
    try {
        ...
        if (interaction.customId === 'panel_form_selector') {
            ...
            let selectedNames = [...new Set(raw.split(',').map(s => s.trim()).filter(s => s.length > 0))];
            ...
            const selectMenu = new StringSelectMenuBuilder()
                ...
                .addOptions(validNames.map(name => ({ label: name, value: name, description: 'Formulario: ' + name })));
            const row = new ActionRowBuilder().addComponents(selectMenu);
            const embed = new EmbedBuilder()
                .setTitle('Panel de Formularios')
                .setDescription('Selecciona un formulario para comenzar.')
                .setColor('#00aaff')
                .setTimestamp();
            await interaction.channel.send({ embeds: [embed], components: [row] });
            let replyMsg = '✅ Panel creado con: ' + validNames.join(', ');
            if (invalidNames.length) replyMsg += '\n⚠️ No existen y se ignoraron: ' + invalidNames.join(', ');
        ...
        if (interaction.customId === 'crear_form_modal') {
            ...
            forms[nombre] = { nombre, creadoPor: interaction.user.id, creadoEn: Date.now() };
