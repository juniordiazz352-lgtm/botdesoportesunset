...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Configurar bienvenida (canal, mensaje, roles, imagen)')
        ...
        .addStringOption(opt => opt.setName('imagen').setDescription('URL imagen de fondo').setRequired(false)),
    ...
    async execute(interaction) {
        ...
        // Procesar roles: puede ser mención o ID
        let roleIds = [];
        if (rolesRaw) {
            const matches = rolesRaw.match(/<@&(\d+)>|\d+/g);
            if (matches) {
                roleIds = matches.map(m => m.replace(/\D/g, ''));
            }
        }
