...
module.exports = {
    ...
    async execute(interaction) {
        // Aplazar respuesta efímera (solo el usuario ve el mensaje de confirmación)
        await interaction.deferReply({ ephemeral: true });

        const user = interaction.user;
        const dm = await user.createDM().catch(() => null);
        if (!dm) {
            return interaction.editReply({ content: '❌ No pude enviarte DM. Habilita los mensajes directos.' });
        }
        ...
        await interaction.editReply({ content: '✅ Revisa tus mensajes directos.' });
    }
};
